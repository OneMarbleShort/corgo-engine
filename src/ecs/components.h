//
//  ecs/components.h
//  Components.h is a header file that includes the list of all component types, including core, engine, and game-defined components.
//  Gameplay code should only use this
//  Copyright (c) 2026 Carlos Camacho. All rights reserved.
//

#ifndef CORGO_ECS_COMPONENTS_H
#define CORGO_ECS_COMPONENTS_H

#include "types.h"
#include "core/component.h"

// Include engine config to get the game directory 
#include "../engine/config.h"

//// Build component list
// Core ECS components
#include "core/core_components.h"
#ifndef CE_COMPONENT_DESC_CORE
#define CE_COMPONENT_DESC_CORE(CE_COMPONENT_DESC)
#endif
#ifndef CE_GLOBAL_COMPONENT_DESC_CORE
#define CE_GLOBAL_COMPONENT_DESC_CORE(CE_GLOBAL_COMPONENT_DESC)
#endif

// Engine components
#include "../engine/components.h"
#ifndef CE_COMPONENT_DESC_ENGINE
#define CE_COMPONENT_DESC_ENGINE(CE_COMPONENT_DESC)
#endif
#ifndef CE_GLOBAL_COMPONENT_DESC_ENGINE
#define CE_GLOBAL_COMPONENT_DESC_ENGINE(CE_GLOBAL_COMPONENT_DESC)
#endif

// Game components
#include CE_GET_GAME_INCLUDE(CE_ENGINE_GAME_DIR, components)
#ifndef CE_COMPONENT_DESC_GAME
#define CE_COMPONENT_DESC_GAME(CE_COMPONENT_DESC)
#endif
#ifndef CE_GLOBAL_COMPONENT_DESC_GAME
#define CE_GLOBAL_COMPONENT_DESC_GAME(CE_GLOBAL_COMPONENT_DESC)
#endif

//// Generate component Types Enum
typedef enum CE_COMPONENT_TYPES_ENUM {
#define CE_COMPONENT_DESC(name, uid, storage, initial_capacity, ...) name,
	CE_COMPONENT_DESC_CORE(CE_COMPONENT_DESC)
	CE_COMPONENT_DESC_ENGINE(CE_COMPONENT_DESC)
#ifndef CE_CORE_TEST_MODE
	CE_COMPONENT_DESC_GAME(CE_COMPONENT_DESC)
#endif
#undef CE_COMPONENT_DESC
	CE_COMPONENT_TYPES_COUNT //Invalid component count
} CE_COMPONENT_TYPES;

// Sanity check: ensure we don't exceed the bitmask capacity.
_Static_assert(CE_COMPONENT_TYPES_COUNT <= CE_MAX_COMPONENT_TYPES, "Too many component types: exceeds MAX_COMPONENT_TYPES");

//// Declare component functions
#define CE_COMPONENT_DESC(name, uuid, storage, initial_capacity, ...) CE_DECLARE_COMPONENT(name, uuid, storage, initial_capacity)
	CE_COMPONENT_DESC_CORE(CE_COMPONENT_DESC)
	CE_COMPONENT_DESC_ENGINE(CE_COMPONENT_DESC)
#ifndef CE_CORE_TEST_MODE
	CE_COMPONENT_DESC_GAME(CE_COMPONENT_DESC)
#endif
#undef CE_COMPONENT_DESC

//// Global components
//// Generate component Types Enum
typedef enum CE_GLOBAL_COMPONENT_TYPES_ENUM {
#define CE_GLOBAL_COMPONENT_DESC(name, storage) CE_GLOBAL_COMPONENT(name),
	CE_GLOBAL_COMPONENT_DESC_CORE(CE_GLOBAL_COMPONENT_DESC)
	CE_GLOBAL_COMPONENT_DESC_ENGINE(CE_GLOBAL_COMPONENT_DESC)
#ifndef CE_CORE_TEST_MODE
	CE_GLOBAL_COMPONENT_DESC_GAME(CE_GLOBAL_COMPONENT_DESC)
#endif
#undef CE_GLOBAL_COMPONENT_DESC
	CE_GLOBAL_COMPONENT_TYPES_COUNT //Invalid component count
} CE_GLOBAL_COMPONENT_TYPES;

#define CE_GLOBAL_COMPONENT_DESC(name, storage) CE_DECLARE_GLOBAL_COMPONENT(name, storage)
	CE_GLOBAL_COMPONENT_DESC_CORE(CE_GLOBAL_COMPONENT_DESC)
	CE_GLOBAL_COMPONENT_DESC_ENGINE(CE_GLOBAL_COMPONENT_DESC)
#ifndef CE_CORE_TEST_MODE
	CE_GLOBAL_COMPONENT_DESC_GAME(CE_GLOBAL_COMPONENT_DESC)
#endif
#undef CE_GLOBAL_COMPONENT_DESC

//// Debug helpers
const char* CE_ECS_GetComponentTypeNameDebugStr(IN CE_TypeId typeId);
const char* CE_ECS_GetGlobalComponentTypeNameDebugStr(IN CE_TypeId typeId);

#endif // CORGO_ECS_COMPONENTS_H
